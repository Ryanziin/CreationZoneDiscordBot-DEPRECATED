...
public class CheatOnCommand extends CommandCreator {
    
    public CheatOnCommand(){
        super("/", "cheaton", false, null);
    }
    ...
    public void execute(User user, MessageChannel channel, Guild guild, Message message, String[] args){
        ...
        if(cheat != null){
            channel.sendMessage(user.getAsMention() + ", o cheat já está ativado.").complete();
            return;
        }
        
        new RegisterCheat(user);
